...
def policy_walk(env, observations, n_observations, step_size = 0.05, n_iters = 10000): #no idea what a normal step size is - they do 0.05 so I guess this is reasonable 
    ...
    while iters < n_iters: 
        ...
        ratio = calculate_posterior(env,observations,n_observations, R_tild, env.R_max, pi_tild)/calculate_posterior(env,observations, n_observations, R, env.R_max, pi)
        p = min(1,ratio)
        if (random.random() < p):
            R = R_tild 
            values = values_tild 
...
def calculate_posterior(env, observations, n_observations, R, R_max, pi): 
